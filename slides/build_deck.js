...
{
  let s = pres.addSlide();
  addContentTitle(s, "Project Status & Next Steps",
    "32+ commits on github.com/holoros/lsog-ne; ready for Phase 6 the moment Hagan raster lands");
  ...
  const cols = [
    { title: "DELIVERED",
      ...
      items: [
        ...
        "Multi-state (ME/NH/VT/NY)",
        "Bootstrap + FIA design CIs",
        "Ownership × carbon × class breakdown",
        "Pelz cross-validation (Phase 7)",
        "TreeMap wall-to-wall ME (Phase 8 v2)",
        "Integrated NE LSOG report"
      ] },
    { title: "QUEUED — UNBLOCKED",
      color: C.moss,
      items: [
        "Phase 8 v3: Potapov-aware wall-to-wall",
        "Phase 8 NE-debug: gdalwarp -RAT_TABLE",
        "State-specific RH95 thresholds",
        "Maine UT polygon-based filter",
        "rFIA cross-validation",
  ...
  for (let i = 0; i < 3; i++) {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      x, y: 1.55, w: 4.0, h: 5.5, fill: { color: C.white },
      line: { color: C.light_g, width: 1 }
    });
    s.addShape(pres.shapes.RECTANGLE, {
      x, y: 1.55, w: 4.0, h: 0.6, fill: { color: c.color }, line: { color: c.color }
    });
    s.addText(c.title, {
      x: x + 0.2, y: 1.55, w: 3.7, h: 0.6, fontSize: 14, fontFace: "Calibri",
    ...
    const itemList = c.items.map((it, idx) => ({
      ...
      options: { bullet: !(it.startsWith(" ") || it.startsWith("→") || it.trim() === ""),
                 ...
                 fontSize: 12, color: it.startsWith(" ") ? C.muted : C.ink,
                 italic: it.startsWith("→") || it.startsWith(" "),
                 paraSpaceAfter: 4 }
    }));
    s.addText(itemList, {
      x: x + 0.3, y: 2.4, w: 3.6, h: 4.5, fontFace: "Calibri", margin: 0
    });
  }
  ...
  s.addText("Repository: github.com/holoros/lsog-ne   |   Cardinal: /users/PUOM0008/crsfaaron/LSOG/   |   Memory file: CLAUDE.md", {
