# =============================================================================
# Title: FIA LSOG Analysis for New England (v3)
# Author: A. Weiskittel
# Date: 2026-03-18
# Description:
...
data_root <- "/home/aweiskittel/Documents/MAINE/DATA/FIA/ME/"
#
...
out_dir <- file.path(data_root, "LSOG_output")
...
cat("\n")
cat(strrep("=", 70), "\n")
cat(" FIA LSOG Analysis v3: New England\n")
cat(strrep("=", 70), "\n")
...
write_csv(all_results, file.path(out_dir, "fia_lsog_all_states_v3.csv"))
...
for (st in unique(all_results$state)) {
  ...
  ggsave(file.path(out_dir, paste0("lsog_pct_combined_", st, "_v3.png")),
  ...
  ggsave(file.path(out_dir, paste0("lsog_acres_facet_", st, "_v3.png")),
...
if (length(unique(all_results$state)) > 1) {
  ...
  ggsave(file.path(out_dir, "lsog_pct_regional_facet_v3.png"),
  ...
  ggsave(file.path(out_dir, "lsog_pct_state_bar_v3.png"),
  ...
  ggsave(file.path(out_dir, "lsog_acres_state_bar_v3.png"),
  ...
  ggsave(file.path(out_dir, "lsog_heatmap_regional_v3.png"),
...
cat("\n")
cat(strrep("=", 70), "\n")
cat(" COMPARISON: FIA Proxy v3 vs. Hagan et al. (2024) LiDAR\n")
cat(strrep("=", 70), "\n")
...
if (nrow(me_data) > 0) {
  ...
  cat(sprintf("\n  FIA Proxy v3 (%s, all Maine, 17.6M acres):\n",
              me_data$eval_period))
