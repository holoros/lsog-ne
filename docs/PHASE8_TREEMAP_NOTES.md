...
## Phase 8 NE-extension: deeper debug finding (still pending)

Tried gdal_translate -projwin to pre-crop CONUS to NH/VT/NY (mirroring
how Aaron originally produced ME_TM_20.tif). Result: same all-NA TM_ID
mapping. Cropping does NOT preserve the TM_ID encoding the way ME_TM_20.tif
has it.

Hypothesis: ME_TM_20.tif was likely created with a non-standard encoding
that PRESERVES the value attribute table mapping. The CONUS raster's
encoding may use a categorical color table (RAT) that gdal_translate
strips during -projwin crop.

### Three remaining options for next session:

1. **Compare ME_TM_20.tif and gdal-cropped CONUS subset** byte-by-byte
   for the same pixel locations. May show that Aaron used `-rat` or
   similar option to preserve the attribute table.

2. **Use gdalwarp with -ot and explicit category preservation**:
   ```
   gdalwarp -of GTiff -te xmin ymin xmax ymax -t_srs <crs>
            -co RAT_TABLE=YES TreeMap2020_CONUS.tif <state>_TM_20.tif
   ```

3. **Skip wall-to-wall maps and use vat.dbf summed per-state**: TreeMap
   provides Count column in vat.dbf showing pixels per TM_ID across
   CONUS. To get per-state counts we'd need to know which TM_IDs are
   imputed to which states - that's lost without the raster.

   Alternative: use the FIA POP_PLOT_STRATUM_ASSGN tables to determine
   which plots are imputed where.

### Status

- Phase 8 v1 ME 2020/2022: works (commit e9b3a3c, 50% Unknown caveat)
- Phase 8 v2 ME 2020/2022: works (commit 494a632, 0.33% Unknown)
- Phase 8 NE-wide: scaffolding committed (11bb757, 5337370), TM_ID
  mapping bug needs resolution before re-running

ME results stand as the validated TreeMap analysis. Regional extension
to NH/VT/NY remains a clear next-session task with known debug paths.
