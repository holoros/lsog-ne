...
**Fig. 7.** The definitive multi-model LSOG product for Maine. (a) Ensemble mean probability across five learners (balanced random forest, probability forest, weighted logistic, support vector machine, multi-objective support vector regression). (b) Across-model uncertainty, the per-pixel standard deviation among the five models (mean 0.18 on a 0-1 scale); darker areas are where credible models disagree most. (c) The any-LSOG class obtained by calibrating the ensemble probability to the design-based area, so over-prediction is bounded by the unbiased estimate. The five learners' mean mapped probabilities span 0.26 to 0.59, and a variance-based global sensitivity analysis attributes nearly all explained variation to canopy height with the Landsat disturbance axis secondary (Table S7). The probability surface with its uncertainty layer, not the binary class, is the product intended for high-stakes use.
