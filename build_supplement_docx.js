...
k.push(H("Table S4. Seven Islands (SILC) vs reproduced published model",HeadingLevel.HEADING_1));
k.push(csvTable(S+"S5_silc_kappa.csv"));
k.push(cap("Table S4. Agreement between the privately held Seven Islands classification and the reproduced published model, confirming they are effectively the same product."));

k.push(H("Table S5. Cross-method disagreement by owner class",HeadingLevel.HEADING_1));
...
k.push(cap("Table S5. Of the LSOG flagged by any method, the share that is contested (flagged by one or two methods, not all) by owner class. Disagreement is pervasive across ownership."));

k.push(H("Table S6. TreeMap LSOG time series",HeadingLevel.HEADING_1));
...
k.push(cap("Table S6. TreeMap-imputed LSOG area over 2016-2022 (native 30 m), Maine unorganized townships."));

k.push(H("Table S7. LiDAR prediction of structure, and old-growth discriminators",HeadingLevel.HEADING_1));
...
k.push(cap("Table S7a. Cross-validated R-squared for predicting ground structural attributes from the eight LiDAR metrics. Dead wood (CWD, snags) is poorly predicted."));
k.push(csvTable(S+"T3_OG_variable_importance.csv",["variable","set","MeanDecreaseAccuracy"],{MeanDecreaseAccuracy:"importance (MDA)"}));
k.push(cap("Table S7b. Old-growth discriminators (random forest variable importance) across LiDAR and ground-structure variables."));
