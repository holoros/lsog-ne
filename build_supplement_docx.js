...
const tcap=(t)=>new Paragraph({spacing:{before:120,after:50},keepNext:true,children:[new TextRun({text:t,size:18,italics:true,color:GREY})]});
function rnd(s){const n=Number(s); if(!isFinite(n)||s===""||s===undefined) return s; if(Number.isInteger(n)&&n>=1900&&n<=2100) return String(n); return Math.abs(n)>=1000?Math.round(n).toLocaleString():(Math.round(n*1000)/1000).toString();}
function csvTable(path,sel,headerMap){
  ...
  const w=Math.floor(CW/out[0].length); const widths=out[0].map(()=>w); const last=out.length-1;
  return new Table({width:{size:CW,type:WidthType.DXA},columnWidths:widths,
    rows:out.map((r,ri)=>new TableRow({cantSplit:true,children:r.map(c=>new TableCell({borders:bds,width:{size:w,type:WidthType.DXA},
...
k.push(tcap("Table S1. Cross-validated AUC (mean and 95% interval) by classifier (random forest, logistic regression), target, and predictor set. Canopy-height-only is weakest for old growth under both classifiers."));
...
k.push(tcap("Table S2. Design-based older-forest area (large-tree basal area domain) at three thresholds, with 95% CI and 2003-2024 trend. The increasing trend is robust to the threshold."));
...
k.push(tcap("Table S2b. Design-based older-forest area by ownership group with 95% CI and 2003-2024 trend. Older forest is rising on private commercial timberland as well as public land."));
...
k.push(tcap("Table S3. Mean, SD, and range across 20 random forest seeds for reproduced accuracy, wall-to-wall area, and cross-validation agreement."));
...
k.push(tcap("Table S4. Of the LSOG flagged by any method, the share that is contested (flagged by one or two methods, not all) by owner class. Disagreement is pervasive across ownership."));
...
k.push(tcap("Table S5. TreeMap-imputed LSOG area over 2016-2022 (native 30 m), Maine unorganized townships."));
...
k.push(tcap("Table S6a. Cross-validated R-squared for predicting ground structural attributes from the eight LiDAR metrics. Dead wood (CWD, snags) is poorly predicted."));
